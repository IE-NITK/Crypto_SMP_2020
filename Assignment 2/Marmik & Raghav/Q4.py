# Crryptopals assignment q4 :) 
CHARACTER_FREQ = {
    'a': 0.0651738, 'b': 0.0124248, 'c': 0.0217339, 'd': 0.0349835, 'e': 0.1041442, 'f': 0.0197881, 'g': 0.0158610,
    'h': 0.0492888, 'i': 0.0558094, 'j': 0.0009033, 'k': 0.0050529, 'l': 0.0331490, 'm': 0.0202124, 'n': 0.0564513,
    'o': 0.0596302, 'p': 0.0137645, 'q': 0.0008606, 'r': 0.0497563, 's': 0.0515760, 't': 0.0729357, 'u': 0.0225134,
    'v': 0.0082903, 'w': 0.0171272, 'x': 0.0013692, 'y': 0.0145984, 'z': 0.0007836, ' ': 0.1918182
}


def english_score(input_bytes):
   
    score = 0

    for i in input_bytes:
        score += CHARACTER_FREQ.get(chr(i).lower(), 0)

    return score


def singlechar_xor(input_bytes, key_value):
    res = b''

    for i in input_bytes:
        res += bytes([i ^ key_value])

    return res


def brute_force(ciphertext):
    candidates = []

    for key_candidate in range(256):
        plaintext_candidate = singlechar_xor(ciphertext, key_candidate)
        candidate_score = english_score(plaintext_candidate)

        result = {
            'key': key_candidate,
            'score': candidate_score,
            'plaintext': plaintext_candidate
        }

        candidates.append(result)

    
    return sorted(candidates, key=lambda c: c['score'], reverse=True)[0]


def final_result(result):
   
    print(result['plaintext'].decode().rstrip())


def detect_encrypted_text(encrypted_strings):
   
    candidates = []

    for string in encrypted_strings:
        candidates.append(brute_force(string))

    # Return the candidate with the highest English score
    return sorted(candidates, key=lambda c: c['score'], reverse=True)[0]


def main():
    ciphertexts = [bytes.fromhex(line.strip()) for line in open("thisfile.txt")]
    most_likely_plaintext = detect_encrypted_text(ciphertexts)
    final_result(most_likely_plaintext)

    
if __name__ == "__main__":
    main()